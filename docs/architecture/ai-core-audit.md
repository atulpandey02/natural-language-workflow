...
| F9 | **Info/⏳** | **UI: original request not shown; no summary view.** The NL request is client-only; run detail listed raw JSON previews. | observability agent map | ✅ grounded summary endpoint + minimal run-detail summary panel added; ✅ resolved in the M12B-A addendum: request provenance persisted (RLS, immutable) + shown on the workflow detail page |
