...
test.describe("access + safety guards", () => {
  test("unauthenticated users are redirected to sign in", async ({ page }) => {
    test.skip(!env.baseURL, "requires a running web app");
    await page.goto("/");
    await expect(page).toHaveURL(/\/login/);
  });

  test("REJECT plans cannot be materialized", async ({ page }) => {
    test.skip(!liveStackConfigured, "requires a seeded live stack");
  ...
  test("members cannot see admin approve/reject controls", async ({ page }) => {
    test.skip(!liveStackConfigured || !env.memberEmail, "requires a seeded member account");
  ...
  test("double-click Run now creates exactly one durable run", async ({ page }) => {
    test.skip(!liveStackConfigured, "requires a seeded live stack + a materialized workflow");
    await signIn(page, env.adminEmail, env.adminPassword);
    await page.goto("/workflows");
    await page.getByRole("link").first().click();
    const runNow = page.getByRole("button", { name: /run now/i });
    await runNow.click();
    // The button disables during the in-flight request; the shared idempotency
    // key guarantees the backend returns one durable run regardless.
    await expect(page).toHaveURL(/\/runs\/[0-9a-f-]{36}$/);
  });

  test("switching workspace does not show stale previous-tenant data", async ({ page }) => {
    test.skip(!liveStackConfigured, "requires two seeded workspaces");
    await signIn(page, env.adminEmail, env.adminPassword);
    await page.goto("/workflows");
    const switcher = page.getByLabel(/select workspace/i);
    const options = await switcher.locator("option").all();
    test.skip(options.length < 2, "needs at least two workspaces");
