...
#      do not;
#   8  a SECOND release (N -> N+1: no-op migration 0021, same keys) rolled out
#      THROUGH <ops_root>/current with target.env pointing at it — no duplicate
...
log "9b/10 SECOND release (N -> N+1) rolled out THROUGH current: no-op migration 0021, same keys, operator Alertmanager kept"
NEXT_REV="0021_rehearsal_noop"
...
cat > "$SRC/migrations/versions/${NEXT_REV}.py" <<EOF
"""rehearsal_noop — DISPOSABLE no-op migration for the N -> N+1 rehearsal only.
...
def downgrade() -> None:
    pass
EOF
git -C "$SRC" add -A >/dev/null && git -C "$SRC" -c user.name=rehearsal -c user.email=r@localhost commit -q -m "rehearsal: N+1 no-op migration"
...
"${GEN[@]}" generate --target-env "$TMP/target.env" --release-sha "$NEW2_SHA" --backend-image "$NEW2_BACKEND" --web-image "$NEW2_WEB" \
  --generated-by local-rehearsal --target-revision "$NEXT_REV" --out "$MANIFEST2" >/dev/null
...
[ "$(psql_owner "SELECT version_num FROM alembic_version")" = "$NEXT_REV" ] || die "N+1: not at ${NEXT_REV}"
...
ok "N+1 ACTIVATED through current: releases/$NEW_SHA -> releases/$NEW2_SHA, schema ${NEXT_REV}, keys unchanged, operator Alertmanager kept, go-check PASS (fixture record); ${TARGET_HEAD} state was the starting point"
ok "LOCAL TIMING: N+1 preflight -> reopen took $(( $(date +%s) - T2_START )) s"
TARGET_HEAD="$NEXT_REV"   # the downgrade proof below starts from the N+1 head
